...
You can check your tunnel's connection status either from Cloudflare One (by going to **Networking** > **Tunnels**) or by running the `cloudflared tunnel list` command. Each tunnel displays a status that reflects its current connection state:
